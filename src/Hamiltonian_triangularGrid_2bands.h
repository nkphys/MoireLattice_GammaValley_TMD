...
void Hamiltonian::Initialize(){
    ...
    Wnr_center_1.resize(2);Wnr_center_2.resize(2);
    Wnr_center_x.resize(2);Wnr_center_y.resize(2);
...
for(int r1_ind=0;r1_ind<space_slices;r1_ind++){
        ...
        Overlaps_[m][n] += d_r1*d_r2*conj(Psi_state_[m][r_ind])*Gaussian(rx_, ry_, Wnr_center_x[n], Wnr_center_y[n], std_dev);
...
//Create Unitary_opr
for(int band_n=0;band_n<2;band_n++){
for(int band_l=0;band_l<2;band_l++){
Unitary_opr[band_n][band_l]=0;
for(int band_m=0;band_m<2;band_m++){
...
int center_neigh;
for(int band_alpha=0;band_alpha<2;band_alpha++){
for(int band_beta=0;band_beta<2;band_beta++){
for(int r2_=0;r2_<L2_eff;r2_++){
	...
	for(int band_alpha_p=0;band_alpha_p<2;band_alpha_p++){
...
*/
  ...
  cout<<"Checking Wannier functions overlaps-----------------------"<<endl;
   ...
   cout<<"----------------------------------------------------------"<<endl;
    ...
    cout<<"--------------------------------------Tij[1][0][center][neigh]--------------------------------------"<<endl;


     for(int r2_=0;r2_<L2_eff;r2_++){
        for(int r1_=0;r1_<L1_eff;r1_++){
            center_neigh = (r1_) + L1_eff*(r2_);
            cout<<Tij[1][0][center_][center_neigh]<<"  ";
        }
        cout<<endl;
    }
 ...
 cout<<"--------------------------------------Tij[1][1][center][neigh]--------------------------------------"<<endl;


     for(int r2_=0;r2_<L2_eff;r2_++){
        for(int r1_=0;r1_<L1_eff;r1_++){
            center_neigh = (r1_) + L1_eff*(r2_);
            cout<<Tij[1][1][center_][center_neigh]<<"  ";
        }
        cout<<endl;
    }
